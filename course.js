...
// Restore teaching scenarios across lesson/ER visits without implying course completion.
try{const saved=JSON.parse(localStorage.getItem('mes-scenarios:v54:'+courseKey)||'{}');for(const [id,v] of Object.entries(saved)){const s=SCENARIOS[courseKey].find(x=>x.id===id);if(s&&s.nodes[v.nodeId]){const e=new ScenarioEngine(s);e.nodeId=v.nodeId;e.events=Array.isArray(v.events)?v.events:e.events;e.feedback=v.feedback||e.feedback;e.kind=v.kind||'';engines.set(id,e)}}}catch{}
window.addEventListener('pagehide',()=>{try{localStorage.setItem('mes-scenarios:v54:'+courseKey,JSON.stringify(Object.fromEntries([...engines].map(([id,e])=>[id,{nodeId:e.nodeId,events:e.events,feedback:e.feedback,kind:e.kind}]))))}catch{}});
let lessonIndex=0,activeScenario=SCENARIOS[courseKey][0].id;
const esc=value=>String(value??'—').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
$('#course-root').innerHTML=`<header class="topbar"><a class="brand" href="index.html"><b>M</b><span><small>MES LEARNING LAB</small>從現場認識資料</span></a><div class="course-tabs"><a href="index.html">01 基本物件</a><a href="operations.html" ${courseKey==='operations'?'aria-current="page"':''}>02 設備與現場</a><a href="flow.html" ${courseKey==='flow'?'aria-current="page"':''}>03 Flow 與 Lot</a><a href="freshness.html">04 資料新鮮度</a></div><button class="button mobile-menu" id="menu-toggle" aria-expanded="false" aria-controls="sidebar">課程目錄</button></header><div class="shell"><aside id="sidebar"><p class="eyebrow">${course.label} · ${course.short}</p><h2 class="sidebar-title">${course.title}</h2><nav aria-label="本課章節">${course.lessons.map((l,i)=>`<a href="#${l.id}" data-lesson="${l.id}"><span>${String(i+1).padStart(2,'0')}</span><div>${l.nav}<small>${l.sub}</small></div></a>`).join('')}</nav><div class="aside-foot"><p>先讀概念，再操作情境。<br>每個練習都可重設與比較。</p><a href="index.html">第一部分 · 基本物件</a><br><a href="operations.html">第二部分 · 設備現場</a><br><a href="flow.html">第三部分 · Flow 深入</a><br><a href="freshness.html">第四部分 · 資料新鮮度</a><br><a href="integrated-map.html?view=original">ER 整合大圖</a><br><a href="integrated-map.html?view=questions">跨圖聚焦題目</a><br><a href="data-map.html?view=original#2676" data-er-sidebar>ER 圖 · 原圖</a><br><a href="data-map.html?view=questions#2676">ER 圖 · 聚焦題目</a></div></aside><main id="content" tabindex="-1"><div class="crumb">MES 入門教學 ／ ${course.label}</div><div class="page-meta"><span class="pill">${course.short}</span><span id="progress"></span></div><div id="lesson-container"></div><div class="footer-nav"><button class="button" id="previous">← 上一段</button><span id="footer-progress"></span><button class="button primary" id="next">下一段 →</button></div><p class="page-foot"><a href="${course.next}">${course.nextText} →</a></p>${sources}</main></div><dialog id="image-dialog" aria-label="完整教學圖"><div class="dialog-bar"><span>完整教學圖</span><button class="button" id="zoom-toggle">原始尺寸</button><button class="button" id="close-dialog">關閉 ×</button></div><div class="zoom-view"><img id="zoom-image" alt="放大的教學示意圖"></div></dialog>`;
...
function stateView(s){
 ...
 if(courseKey==='operations'){
  ...
  if(s.current==='結束')result+='<p class="completion">✓ 本教學 Flow 已完成；這不表示 Lot E 已出貨。</p>';
 }
