...
out=Path('tests/evidence/mes-041');out.mkdir(parents=True,exist_ok=True)
with sync_playwright() as p:
 ...
 for width in [1440,390]:
  ...
  for ref in ['2673:stream','2673:stream-link']:
   page.evaluate('(r)=>ER_ATLAS.selectNode(ER_ATLAS.model.mapping[r])',ref)
   for term in ['Flow','WPH','LDS']:assert term in page.locator('#meaning').text_content()
  page.locator('#find').fill('Stream_fcst');nid=page.evaluate('ER_ATLAS.model.mapping["2673:stream"]')
  ...
  page.locator('#open-teaching').click();assert page.locator('#teaching-large').get_attribute('src').endswith('stream-arrival-mobile-v1.png' if width==390 else 'stream-arrival-v2.png')
  ...
  page.close()
 b.close()
print('PASS Stream: entity/relationship, search click, desktop/mobile lesson and enlargement')
