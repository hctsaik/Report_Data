...
out=Path('tests/evidence/mes-042');out.mkdir(parents=True,exist_ok=True)
with sync_playwright() as p:
 ...
 for width in [1440,390]:
  ...
  for ref in ['2673:forecast','2673:forecast-link']:
   page.evaluate('(r)=>ER_ATLAS.selectNode(ER_ATLAS.model.mapping[r])',ref)
   for term in ['Flow','WPH','預計到站時間']:assert term in page.locator('#meaning').text_content()
  page.locator('#find').fill('dm_tbl_lot_forecast');nid=page.evaluate('ER_ATLAS.model.mapping["2673:forecast"]')
  ...
  page.locator('#open-teaching').click();assert page.locator('#teaching-large').get_attribute('src').endswith('lot-forecast-mobile-v1.png' if width==390 else 'lot-forecast-v2.png')
  ...
  page.close()
 b.close()
print('PASS Lot forecast: entity/relationship, search click, desktop/mobile lesson and enlargement')
