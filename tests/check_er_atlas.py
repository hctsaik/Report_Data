...
R=Path(__file__).resolve().parents[1];O=R/'tests/evidence/mes-014/v2';O.mkdir(parents=True,exist_ok=True)
m=json.loads((R/'ER/INTEGRATED/er-model-v2.json').read_text(encoding='utf-8'));NS={'s':'http://www.w3.org/2000/svg'}
svg=ET.parse(R/'ER/INTEGRATED/fab-er-v2.svg');drawn={g.get('id'):g for g in svg.findall('.//s:g',NS)}
...
with sync_playwright() as p:
 ...
 for width in [1440,390]:
  ...
  page.locator('#find').fill('FRCAST_LOT');page.locator('[data-result]').first.click();assert page.locator('#path-list .edge-pair').count()>0
 ...
 b.close()
assert not errors,errors
(O/'verification.json').write_text(json.dumps(dict(canonical_nodes=121,source_nodes=128,visible_edges=128,source_edges=128,originals_unchanged=7,checks=checks,errors=errors),ensure_ascii=False,indent=2),encoding='utf-8')
print('PASS: 128 original nodes/edges mapped to visible graph; 121 canonical nodes; originals unchanged; 17 routes and 5 questions at both widths')
