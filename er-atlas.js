...
function overview(){window.ER_FOCUS.clear();clearFrames();displayMode='overview';document.querySelector('.walk-tools').hidden=true;walkIds=[];document.querySelectorAll('[data-route]').forEach(b=>b.setAttribute('aria-pressed','false'));inspect([],'完整 ER × 主詞關聯','選擇上方主詞或直接點 ER 節點，右側會重新展開來源中的相關實體與關係。現場插圖保留為補充參考。');showTeaching('overview');setView(allBox.slice());svg.querySelectorAll('[data-node],[data-edge]').forEach(g=>g.style.opacity='1');svg.querySelectorAll('[data-edge] path').forEach(p=>{p.style.stroke='#7c8c9c';p.style.strokeWidth='1.7'});$('status').textContent='全圖：121 個節點、128 條關係都在圖上。請搜尋或選一段關係放大閱讀。';}
